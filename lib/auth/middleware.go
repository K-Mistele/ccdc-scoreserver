...
// MIDDLEWARE FOR BLACK TEAM REQUIRED
func BlueTeamRequired(next echo.HandlerFunc) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		// REQUIRE BLACK TEAM OR ADMIN OR BLUE TEAM
		if claims["admin"].(bool) != true  &&
			claims["team"].(string) != string(Black) &&
			claims["team"].(string) != string(Blue){
